        ...
        <div className="relative">
            ...
            <div className="p-2 space-y-6">
                <div className="animate-stagger text-center" style={{ '--stagger-delay': '100ms' } as React.CSSProperties}>
                    ...
                    <p className="text-slate-500 mt-1">Crea y administra las batallas para tus clases.</p>
                </div>
                ...
                <div className="animate-stagger" style={{ '--stagger-delay': '300ms' } as React.CSSProperties}>
                    <h2 className="text-xl font-bold text-slate-700 mb-3">Batallas Creadas</h2>
                    {rooms.length === 0 ? (
                        <p className="text-slate-500 text-center py-8">No hay batallas. ¡Crea una para empezar!</p>
                    ) : (
                        <div className="space-y-4">
                            {rooms.map(room => (
                                <div key={room.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                                    <div className="flex justify-between items-center mb-3">
                                        <div>
                                            <p className="font-bold text-slate-800 text-lg">{room.name}</p>
                                            <p className="text-sm text-slate-500">{room.questionCount} preguntas - {room.groupCount} grupos</p>
                                        </div>
                                        <button
                                    ...
                                    <div className="mt-3 bg-gradient-to-r from-amber-50 to-orange-50 dark:to-orange-900/20 p-4 rounded-lg border-2 border-amber-400 dark:border-amber-600">
                                        <p className="text-xs font-semibold text-slate-700 mb-2">🎮 Código de Batalla (compartir con TODOS los estudiantes):</p>
                                        <p className="font-mono text-3xl font-bold text-amber-600 tracking-widest text-center">{room.battleCode}</p>
                                        <p className="text-xs text-slate-600 mt-2 text-center">Los estudiantes serán asignados automáticamente a equipos</p>
                                    </div>
                                </div>
                            ))}
                        </div>
