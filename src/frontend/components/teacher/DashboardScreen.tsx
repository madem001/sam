...
import { TeacherScreen } from '../../types/types';
...
const ActionCard: React.FC<{ title: string; description: string; icon: React.ReactElement; onClick: () => void, delay: string }> = ({ title, description, icon, onClick, delay }) => (
  <button onClick={onClick} className="bg-white p-6 rounded-2xl shadow-md border border-slate-200 dark:border-slate-700 hover:shadow-lg hover:border-sky-300 dark:hover:border-sky-500 transition-all text-left w-full flex items-center space-x-4 animate-stagger" style={{ '--stagger-delay': delay } as React.CSSProperties}>
    <div className="text-3xl bg-sky-100 text-sky-600 dark:text-sky-400 p-3 rounded-full flex items-center justify-center w-16 h-16">
      {icon}
    </div>
    <div>
