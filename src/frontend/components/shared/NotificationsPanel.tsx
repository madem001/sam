            ...
            <div className={`notifications-panel ${isOpen ? 'open' : ''} flex flex-col`}>
                ...
                <div className="flex-grow overflow-y-auto">
                    {notifications.length === 0 ? (
                        <div className="text-center p-8 text-slate-500 h-full flex flex-col items-center justify-center">
                            {/* FIX: Changed 'className' to 'class' for web component compatibility. */}
                            ...
                            {notifications.map(notif => (
                                <li key={notif.id} className={`p-4 ${notif.read ? 'opacity-60' : ''}`}>
                                    <div className="flex items-start space-x-3">
                                        <div className="text-xl bg-sky-100 text-sky-600 dark:text-sky-400 p-2 rounded-full mt-1">
                                            {/* FIX: Changed 'className' to 'class' for web component compatibility. */}
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            {notif.type === 'battle_invite' && notif.payload && (
                                                <div className="mt-3 flex items-center space-x-2">
                                                    <button onClick={() => handleCopyCode(notif.payload!.roomCode)} className="flex-1 text-xs font-semibold bg-slate-200 text-slate-700 dark:text-slate-200 px-3 py-1.5 rounded-md hover:bg-slate-300 dark:hover:bg-slate-500 transition flex items-center justify-center space-x-1">
                                                        {/* FIX: Changed 'className' to 'class' for web component compatibility. */}
