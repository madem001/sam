...
const BattleLobbyScreen: React.FC<BattleLobbyScreenProps> = ({ onBack }) => {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center h-full text-center p-4 bg-slate-50">
        <button
            onClick={onBack}
      ...
      <div className="w-full max-w-sm animate-stagger" style={{ '--stagger-delay': '200ms' } as React.CSSProperties}>
        ...
        <div className="grid grid-cols-2 gap-4">
          {teamMembers.map((member, index) => (
            <div key={index} className="flex flex-col items-center p-3 bg-white rounded-lg shadow-sm border border-slate-200 dark:border-slate-600">
