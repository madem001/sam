...
const NavItem: React.FC<NavItemProps> = ({ label, iconName, isActive, onClick }) => {
  ...
  const inactiveClasses = 'text-indigo-100 hover:text-white dark:hover:text-white';
...
const TeacherBottomNav: React.FC<TeacherBottomNavProps> = ({ activeScreen, setActiveScreen, enabledModules, customModules }) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 h-24 text-white z-10">
      {/* Background */}
      <div className="absolute bottom-0 w-full h-16 bg-indigo-700 shadow-2xl"></div>
      ...
      {activeIndex !== -1 && (
        ...
        >
            <div className="relative w-full h-full">
                <svg viewBox="0 0 100 96" preserveAspectRatio="none" className="w-full h-full text-indigo-700 drop-shadow-lg" fill="currentColor">
