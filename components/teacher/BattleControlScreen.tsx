  ...
  return (
    <div className="relative h-full overflow-y-auto">
      ...
      <div className="p-2 space-y-6">
        <div className="text-center pt-10">
          ...
          <div className="flex items-center justify-center space-x-2 mt-2">
            <span className={`px-3 py-1 text-sm font-semibold rounded-full ${
              isWaiting ? 'bg-yellow-100 text-yellow-700 dark:text-yellow-300' :
              isActive ? 'bg-green-100 text-green-700 dark:text-green-300' :
        ...
        {isWaiting && (
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 text-center">
            <p className="text-slate-600 mb-4">La batalla está lista. Los estudiantes pueden unirse con sus códigos de grupo.</p>
            <button
        ...
        {isActive && currentQuestion && (
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            <h2 className="text-xl font-bold text-slate-800 mb-4">Pregunta Actual</h2>
            <p className="text-lg text-slate-700 mb-4">{currentQuestion.question_text}</p>
            ...
            <div className="bg-gradient-to-r from-sky-100 to-blue-100 dark:to-blue-900/30 p-4 rounded-lg border-2 border-sky-300 dark:border-sky-700">
              <div className="flex items-center justify-center space-x-2">
                <div className="animate-pulse">
                  <ion-icon name="hourglass-outline" class="text-2xl text-sky-600"></ion-icon>
                </div>
                <p className="text-sky-800 font-semibold">
                  Esperando respuestas de los estudiantes...
                </p>
              </div>
              <p className="text-sm text-sky-700 text-center mt-2">
        ...
        {isFinished && (
          <div className="bg-gradient-to-br from-green-400 to-teal-500 dark:to-teal-600 p-6 rounded-xl shadow-lg text-white text-center">
        ...
        {(isActive || isWaiting) && (
          <div className="flex justify-center">
            <button
              onClick={handleRestartBattle}
              className="px-4 py-2 bg-slate-200 text-slate-700 dark:text-slate-200 font-semibold rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors"
            >
              <ion-icon name="refresh-outline" class="mr-2"></ion-icon>
              Reiniciar Batalla
            </button>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
          <h2 className="text-xl font-bold text-slate-800 mb-4">Ranking de Grupos</h2>
          {groups.length === 0 ? (
            <p className="text-slate-500 text-center py-4">No hay grupos registrados</p>
          ) : (
              ...
              {groups.map((group, idx) => (
                <div
                  ...
                  className={`flex items-center justify-between p-4 rounded-lg ${
                    idx === 0 ? 'bg-amber-50 border-2 border-amber-400' :
                    idx === 1 ? 'bg-slate-50 border-2 border-slate-400' :
                    idx === 2 ? 'bg-orange-50 border-2 border-orange-400' :
                    'bg-slate-50 border border-slate-200 dark:border-slate-600'
                  }`}
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">Código: {group.group_code}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-sky-600">{group.score}</p>
                    <p className="text-xs text-slate-500">{group.correct_answers} correctas</p>
                  </div>
                </div>
              ))}
            </div>
